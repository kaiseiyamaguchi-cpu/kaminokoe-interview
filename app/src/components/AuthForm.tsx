...
const KANPE_SITE_URL =
  process.env.NEXT_PUBLIC_KANPE_SITE_URL ?? "https://kanpe.ai";

function genSessionId(): string {
  // RFC4122 v4 風（簡易・依存なし）
...
export function AuthForm() {
  ...
  const handleProvider = (provider: "google" | "line") => {
    setLoading(provider);
    const sessionId = genSessionId();
    sessionStorage.setItem("kanpe_handoff_session", sessionId);
    const returnTo = `${window.location.origin}/`;
    const url = `${KANPE_SITE_URL}/auth/web-handoff?session=${encodeURIComponent(
      sessionId,
    )}&provider=${provider}&returnTo=${encodeURIComponent(returnTo)}`;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        ...
        <p className="text-center text-[11px] text-[color:var(--text-mute)] leading-relaxed">
          kanpe.ai のログイン画面に移動します
        </p>

        <div className="mt-4 text-center text-[11px] text-[color:var(--text-mute)] leading-relaxed">
          続行すると{" "}
          <a href={`${KANPE_SITE_URL}/legal/terms`} className="underline">利用規約</a>
          {" "}と{" "}
          <a href={`${KANPE_SITE_URL}/legal/privacy`} className="underline">プライバシーポリシー</a>
          に同意したものとみなされます
        </div>
      </div>
    </div>
